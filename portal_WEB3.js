// ==========================================
// portal_WEB3.js - Anchor 智能合約橋接與支付引擎 (架構師終極防彈版)
// ==========================================
...
let isEngineReady = false; // 引擎是否準備就緒
...
// 🛡️ 智能輪詢載入器：等待 CDN 載入完成，避免網路延遲導致的靜默死亡
let retryCount = 0;
const bootInterval = setInterval(() => {
    ...
    if (window.solanaWeb3 && window.anchor && splLib) {
        ...
        PROGRAM_ID = new window.solanaWeb3.PublicKey("B6hzYR6fuxdjWmqLmqku8Yw9kmfqfJ1fxX8U88jmuPVf");
        ADMIN_PUBKEY = new window.solanaWeb3.PublicKey("DV5rUKjo3ufjyAVNpsQaL19GYtToo5oXJpZ6gjvjhDau");
...
window.initWeb3Engine = async function() {
    ...
    if (!solProvider.publicKey) {
        try { await solProvider.connect({ onlyIfTrusted: true }); } catch (e) { return false; }
    }
    if (!solProvider.publicKey) return false;

    const connection = new window.solanaWeb3.Connection("https://api.devnet.solana.com", "confirmed");
    currentProvider = new window.anchor.AnchorProvider(connection, solProvider, { preflightCommitment: "confirmed" });
    window.anchor.setProvider(currentProvider);
    anchorProgram = new window.anchor.Program(IDL, PROGRAM_ID, currentProvider);
    
    window.syncHUD();
    setInterval(window.syncHUD, 1500); // 啟動實時掃描
    return true;
};

window.syncHUD = async function() {
    if (!isEngineReady || !anchorProgram || !currentProvider || !currentProvider.wallet.publicKey) return;
    const walletPk = currentProvider.wallet.publicKey;
    const splLib = window.splToken || window.solanaSplToken;

    const [userRecordPda] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("user_record"), walletPk.toBytes()], PROGRAM_ID);
    const [dustMint] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("dust_mint")], PROGRAM_ID);
    const userDustAccount = splLib.getAssociatedTokenAddressSync(dustMint, walletPk);
...
window.web3StakeML1 = async function() {
    ...
    const walletPk = currentProvider.wallet.publicKey;
    const rawAmount = new window.anchor.BN(Math.floor(amt).toString());
    const splLib = window.splToken || window.solanaSplToken;

    const [globalMl1Vault] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("global_ml1_vault")], PROGRAM_ID);
    const [ml1Mint] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("ml1_mint")], PROGRAM_ID);
    const [userRecordPda] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("user_record"), walletPk.toBytes()], PROGRAM_ID);
    const userMl1Account = splLib.getAssociatedTokenAddressSync(ml1Mint, walletPk);
    ...
    try {
        ...
        try {
            ...
            tx.add(await anchorProgram.methods.initUserRecord().accounts({ owner: walletPk, userRecord: userRecordPda, systemProgram: window.solanaWeb3.SystemProgram.programId }).instruction());
        }

        // 幫玩家把自己的 ATA 加入交易 (防止玩家把錢包的代幣轉空過而報錯)
        tx.add(splLib.createAssociatedTokenAccountIdempotentInstruction(walletPk, userMl1Account, walletPk, ml1Mint));

        tx.add(await anchorProgram.methods.stakeMl1(rawAmount).accounts({
            owner: walletPk, userRecord: userRecordPda, userMl1Account: userMl1Account, globalMl1Vault: globalMl1Vault, tokenProgram: splLib.TOKEN_PROGRAM_ID
        }).instruction());
        ...
        console.error(err);
        if (typeof typeWriter === 'function') typeWriter(`> ERROR: 操作遭拒絕`, `> 錯誤: 操作遭拒絕`, "var(--alert-red)");
    }
};
...
window.web3ClaimO2 = async function() {
    ...
    const [globalState] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("global_state")], PROGRAM_ID);
    const [o2Mint] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("o2_mint")], PROGRAM_ID);
    const [userRecordPda] = window.solanaWeb3.PublicKey.findProgramAddressSync([utf8.encode("user_record"), walletPk.toBytes()], PROGRAM_ID);
    const userO2Account = splLib.getAssociatedTokenAddressSync(o2Mint, walletPk);

    if (typeof typeWriter === 'function') typeWriter("> EXTRACTING ACCUMULATED $O2 FUEL...", "> 正在提取並結算累積的 $O2 燃料...", "var(--lag-blue)");

    try {
        const tx = new window.solanaWeb3.Transaction();
        tx.add(splLib.createAssociatedTokenAccountIdempotentInstruction(walletPk, userO2Account, walletPk, o2Mint));
        tx.add(await anchorProgram.methods.claimO2().accounts({
            owner: walletPk, userRecord: userRecordPda, globalState: globalState, o2Mint: o2Mint, userO2Account: userO2Account, tokenProgram: splLib.TOKEN_PROGRAM_ID
        }).instruction());
        ...
        console.error(err);
        if (typeof typeWriter === 'function') typeWriter(`> CLAIM FAILED`, `> 提取失敗: 尚未累積或取消`, "var(--alert-red)");
    }
};
...
window.web3BurnForDust = async function() {
    ...
    const walletPk = currentProvider.wallet.publicKey;
    const amtBn = new window.anchor.BN(Math.floor(amt).toString());
    const decimalsBn = new window.anchor.BN("1000000000"); 
    ...
    const userO2Account = splLib.getAssociatedTokenAddressSync(o2Mint, walletPk);
    const userDustAccount = splLib.getAssociatedTokenAddressSync(dustMint, walletPk);
    const adminO2Account = splLib.getAssociatedTokenAddressSync(o2Mint, ADMIN_PUBKEY);
    ...
    try {
        const tx = new window.solanaWeb3.Transaction();
        tx.add(splLib.createAssociatedTokenAccountIdempotentInstruction(walletPk, userDustAccount, walletPk, dustMint));
        tx.add(await anchorProgram.methods.burnO2ForDust(rawAmount).accounts({
            owner: walletPk, globalState: globalState, o2Mint: o2Mint, dustMint: dustMint, userO2Account: userO2Account, userDustAccount: userDustAccount, adminO2Account: adminO2Account, tokenProgram: splLib.TOKEN_PROGRAM_ID
        }).instruction());
